...
class AlterViewStorePreorders extends Migration
{
    /**
     * Run the migrations.
     *
...
FROM
    ...
    FROM
        ((`store_preorder`
    LEFT JOIN`store_preorder_details` `t2` ON (((`store_preorder`.`store_preorder_code` = `t2`.`store_preorder_code`)
        AND (`t2`.`deleted_at` IS NULL))))
    LEFT JOIN`warehouse_preorder_products` `t1` ON ((`t2`.`warehouse_preorder_product_code` = `t1`.`warehouse_preorder_product_code`)))) `dt1`
