...
use App\Modules\Store\Mails\StoreEmailVerificationMail;
...
class StoreOtpService
{
    ...
    public function generatePhoneVerificationOTP(){
        try{
            $user = auth()->user();
            if(!$user->store){
                throw new Exception('user should be store only');
            }
    ...
    private function sendMail($user,$otpData){
        ...
        SendMailJob::dispatch($user->login_email,new StoreEmailVerificationMail($data));
    }
