...
namespace App\Modules\AlpasalWarehouse\Repositories\AdminCurrentStock;


use App\Modules\AlpasalWarehouse\Models\WarehouseProductMaster;
use Illuminate\Support\Facades\DB;

class WarehouseWiseStockReportingRepository
{

    public function getWarehouseWiseCurrentStock($filterParameters,$paginatedBy = 10)
    {
        $warehouseWiseCurrentStocks = WarehouseProductMaster::select(
            'warehouse_product_master.warehouse_product_master_code',
            'warehouses.warehouse_name',
            'warehouses.warehouse_code'
        )
            ->join('warehouses',function($join) use($filterParameters){
                $join->on('warehouse_product_master.warehouse_code','warehouses.warehouse_code')
...
//            })
            ...
            ->groupBy('warehouses.warehouse_code')
            //->get();
            ->paginate($paginatedBy);
        //dd($vendorWiseCurrentStocks);
        return $warehouseWiseCurrentStocks;
    }
