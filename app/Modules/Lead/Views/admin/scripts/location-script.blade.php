<script>
    ...
    function wardChange() {
        var selectedLocationCode = $('#ward option:selected').val();
        var tole_street = "{{ isset($locationPath) ? $locationPath['tole']->location_code : '' }}";
        var locationType = 'tole_street';
        if (selectedLocationCode) {
            $('#tole_street').empty();
            $.ajax({
                type: 'GET',
                ...
                $('#tole_street').append('<option value="" selected >--Select An Option--</option>');
                response.data.forEach(function(locationHierarchy) {
                    $('#tole_street').append('<option ' + ((locationHierarchy.location_code ===
                        tole_street) ?
                            "selected" : '') + ' value="' + locationHierarchy.location_code + '">' +
                        locationHierarchy.location_name + '</option>');
                });
