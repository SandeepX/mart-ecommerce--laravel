<script>
    ...
    $('#formRespondSubscription').submit(function (e) {
        e.preventDefault();
        Swal.fire({
            title: 'Are you sure you want to Respond?',
            text: 'Both payment and subscription will be responded',
            ...
            if (result.isConfirmed) {
                saveRespontToPayment();
            }
        })
    });

    function saveRespontToPayment(){
        var formAction = $('#formRespondSubscription').attr('action');
        var formMethod = $('#formRespondSubscription').attr('method');

        let respondSubscriptionData = new FormData(document.getElementById("formRespondSubscription"));
        $.ajaxSetup({
            headers:
