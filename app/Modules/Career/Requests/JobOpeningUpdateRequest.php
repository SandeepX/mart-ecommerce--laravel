...
class JobOpeningUpdateRequest extends FormRequest
{
    /**
    ...
    {
        $jobOpeningCode =$this->route('job_opening');
        $validJobTypes = JobOpeningService::getAllJobTypesValue();

        return [
            'title' => ['required','max:191',Rule::unique('job_openings','title')->ignore($jobOpeningCode,'opening_code')],
            'slug' => ['required','max:191',Rule::unique('job_openings','slug')->ignore($jobOpeningCode,'opening_code')],
