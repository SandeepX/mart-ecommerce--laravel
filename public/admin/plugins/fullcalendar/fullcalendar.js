...
// TODO: rename everything "lang" to "locale", like what the moment admin did
...
// Make the mouse cursor express that an event is not allowed in the current areas
...
function formatRangeWithChunks(date1, date2, chunks, separator, isRTL) {
	...
	// The areas in the middle is different for both of the dates.
...
var Popover = Class.extend({
	...
	position: function() {
		...
		// if the window is scrolled, it causes the visible areas to be further down
...
var DayGrid = Grid.extend({
	...
	bottomCoordPadding: 0, // hack for extending the hit areas for the last row of the coordinate grid
...
var View = fc.View = Class.extend({
    ...
    /* Top toolbar areas with buttons and title
----------------------------------------------------------------------------------------------------------------------*/
...
fcViews.agenda = View.extend({ // AgendaView
	...
	initialize: function() {
		...
		if (this.opt('allDaySlot')) { // should we display the "all-day" areas?
	...
	render: function() {
		...
		if (this.dayGrid) {
			...
			// have the day-grid extend it's coordinate areas over the <hr> dividing the two grids
	...
	// Generates the HTML that goes before the bg of the TimeGrid slot areas. Long vertical column.
	...
	setHeight: function(totalHeight, isAuto) {
		...
		// limit number of events in the all-day areas
		if (this.dayGrid) {
			this.dayGrid.destroySegPopover(); // kill the "more" popover if displayed
	...
	renderEvents: function(events) {
		...
		// the all-day areas is flexible and might have a lot of events, so shift the height
		this.updateHeight();
	},
    ...
    /* A week view with an all-day cell areas at the top, and a time grid below
----------------------------------------------------------------------------------------------------------------------*/
...
fcViews.agendaWeek = {
    ...
    /* A day view with an all-day cell areas at the top, and a time grid below
----------------------------------------------------------------------------------------------------------------------*/
