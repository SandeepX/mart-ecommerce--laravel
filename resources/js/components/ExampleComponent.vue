<template>
    <div class="container-fluid subtitle-1 pa-3">
        <div v-if="filterList">
            <div class="row pa-5 grey_background">
                ...
                <div v-if="show422Error" class="col-lg-12">
                    <div class="alert alert-info alert-dismissible fade in">
                        <span v-for="(error422,index) in show422ErrorMessage" :key="index"
                              class="glyphicon glyphicon-info-sign mb-1"
                              style="font-size: 18px;">{{ error422['message'] }}</span>
                    </div>
                </div>
                    ...
                    <div class="mb-5 pa-2 subtitle-1 text-uppercase grey_background">
                        <div class="row">
                            ...
                            <div class="col-lg-2" style="margin-top: -2px !important;">
                                <button @click="createPurchaseOrderList" type="button" class="btn btn-primary btn-xs">+
                                    Add
                                </button>
                            </div>
                        </div>
                        ...
                        <div class="col-lg-5">
                            ...
                            <v-select
                                ...
                                <li slot="list-footer" class="pagination">
                                    <button @click="loadMoreProductList" :disabled="!hasMoreProducts">Load More</button>
                                </li>
                            </v-select>
                        </div>
                        <div class="col-lg-3">
                            <div v-if="showVariantList" class="mb-2 subtitle-2 font-weight-bold">Variant Name :</div>
                            <v-select
            ...
            <div class="col-lg-3 pa-0">
                <div class="pa-3 text-center" v-if="purchaseOrder.length>0"
                    ...
                    <label class="radio-inline"><input @click="setPaymentMethod('cash')" type="radio"
                                                       name="payment_method" checked style="margin-top: 0 !important;">Cash</label>
        ...
        <table v-if="purchaseOrder.length>0" class="table v-card">
            ...
            <tr v-for="product in purchaseOrder" :key="product['purchase_order_code']">
                <td class="subtitle-2 text-center">
                    ...
                    <div class="mt-1">
                        <button @click="removePurchaseOrderFromTheList(product['purchase_order_code'])" type="button"
                ...
                <td class="subtitle-2">
                    ...
                    <div v-else-if="product['scheme']['scheme_name']==='Item Scheme'">
                        <div v-if="product['scheme']['scheme_type']==='same'">
                            ...
                            <div class="mt-2 text-right" v-if="product['scheme']['selected_same_item_scheme']===null">
                                <a href="#" data-toggle="tooltip" data-placement="bottom"
        ...
        <div v-if="purchaseOrder.length===0" class="v-card pa-10 subtitle-2 text-center">No Purchase Order Available.
        </div>
        <div v-if="this.addNewPurchaseOrder.length>0" class="subtitle-1 grey_background v-card pa-3 font-weight-bold">
            <div class="row">
                <div class="col-lg-10 mt-2 text-center"> Total Cost Price : Rs. {{ totalNetCP.toFixed(2) }}</div>
                <div class="col-lg-1 text-right">
                    <button @click="savePurchaseOrder('draft')" type="button" class="btn btn-primary btn-sm">Save as Draft
                    </button>
                </div>
                <div class="col-lg-1 text-right">
                    <button @click="savePurchaseOrder('save')" type="button" class="btn btn-primary btn-sm">Save Changes
                    </button>
                </div>
            </div>
        </div>
        <div v-if="showSaveAlert" class="col-lg-12">
            <div class="alert fade in" :class="success?'alert-success':'alert-danger'"
                 style="position:absolute !important;bottom:0 !important;right:10px !important">
                <span class="glyphicon glyphicon-info-sign mr-3" style="font-size: 18px;"></span>{{
                    showSaveAlertMessage
                }}
            </div>
        </div>
        <DialogBox :offered_scheme="offeredSelectedDifferentItem" v-if="showModal && offeredSelectedDifferentItem"
...
export default {
    ...
    methods: {
        ...
        getFilterDetailForPurchaseOrder() {
            ...
            Api.get("/warehouse/warehouse-purchase-orders/create", {})

                .then(response => {

                    this.filterDetail = response.data.data;
        ...
        createPurchaseOrderList() {
            ...
            if (this.selectedProductName && this.selectedVariantName && this.selectedPackagingTypeName || !this.selectedProductName['has_variants']) {
                ...
                let url = '/warehouse/warehouse-purchase-orders/filter-vendor-products/' + this.selectedProductName['product_code'] + '/show';

                //Hit The Api To Get The Cost Price Details along with discount amount.
                Api.getResponseWithParams(url, params, {}).then((response) => {

                    this.selectedProductSelectionInformation = response.data.data;
        ...
        loadMoreProductList() {
            ...
            if (this.selectedVendor) {
                ...
                if (this.currentPage < this.pagination['last_page']) {
                    ...
                    Api.getResponseWithParams(
                        "/warehouse/warehouse-purchase-orders/filter-vendor-products", params, {}
                    )
                        .then(response => {

                            this.productLists = this.productLists.concat(response.data.data);
        ...
        getProductList(loading, search) {
            ...
            if (this.selectedVendor) {
                ...
                Api.getResponseWithParams(
                    ...
                    /* Api.getResponseWithParams("/vendor/schemeable-products", params, {
                         'Accept': 'application/json',
                         'X-Requested-with': 'XMLHttpRequest',
                         'Authorization': 'Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJSUzI1NiJ9.eyJhdWQiOiI3IiwianRpIjoiMzljYmM1MzBiZGFmMDljYmJjYmU3MzQ3M2UxYmY0ZWIwMWEwMmFlMTA1NWRmNzNkNzI5ZjQ0OTMxODhjMTQ5ZTA3MzIxZDZkMTk1NWUyOGMiLCJpYXQiOjE2MjMzMjQzNzYsIm5iZiI6MTYyMzMyNDM3NiwiZXhwIjoxNjIzNDEwNzc2LCJzdWIiOiJVMDAwMDAwMDIiLCJzY29wZXMiOltdfQ.6kqFDLdup0vwdSckW-I7fHpsZpEtQZclXadsOXnGErZMB96FC31VErqPbUqL9CUOZXwh5zv1Bx4d1WTvfpaZpuIfQaku75KCfQ5itFsHUn3N75uglhaLNpYHatR8JsyGtryj9KdpEn_gzJ3XVW95GyGmj-eYkZjUpuhr_L7uD1oOmUqKjFd_4WEj9kRkkHtOwr5ZCCmetXMX-zklJq28YKGGKUW0wD0PcfeHRWqijCA4vpaUWlB5Ti9G5mBm8Myydu3lmY5476hDdIGokLJZ-6HwmIDLAelXvmtbR9muzbJC9pRviCHw4tDC-3Xf0ZTkeZAbG7Df_LON3xRA3JdNC3ZLgEFaLr7ybVDFeLjKXGLkk6Y5p44uooaRZdl7_IYJFz3aOSyryf1a73L2kDC6Xvorc_0Lgr_mca4wmF_i3YC6NJdXNezRxIOjEIFZC0Zg31gLmSELdrtn3tHdZhF9QdfVSqp83VzBl3TBk1_qP3J4xg5UoMrDms7huwHbSf-bPzAf1G2EJIM0tSWRKfyBAYeXcymI8X84vrMNmxjNWHZi_7S49OJTbmpB79d3x4UV2lATIzM5C5YjBzn3v-Ax9KkxYZTt0N4mwhZxNa08un68xQPrVjw014SL8bhbG9JAzCsFAnP7EiVt1uDuU4oykNnpcnAzBMnpTkhDmU6qbDg',
                     })*/
                    .then(response => {
        ...
        savePurchaseOrder(saveType) {
            ...
            for (let i = 0; i < this.purchaseOrder.length; i++) {
                ...
                if (this.purchaseOrder[i]['variant_name']['product_variant_code']) {
            ...
            Api.post("/warehouse/warehouse-purchase-orders/store", fd, {}).then(response => {
                ...
                this.showSaveAlertMessage = response.data.data;
